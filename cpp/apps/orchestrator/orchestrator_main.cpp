...
int main(int argc, char *argv[])
{
    ...
    try
    {
        ...
        OrchestratorEngine orchestrator_engine(config);

        // Start all components
        if (!orchestrator_engine.start())
        {
            std::cerr << "Failed to start orchestrator engine\n";
            return 1;
        }

        Logger::info("Orchestrator engine is running. Press Ctrl+C to stop.");
