...
enum class MessageType
{
    ...
    KV_CHUNK_ARRIVED, // Generated by RdmaPoller when chunk received (DEPRECATED)

    // NEW: Layer-based transfer messages
    KV_LAYER_ARRIVED,       // Single layer arrived
    LAYER_READY_FOR_DECODE, // Can start decode (first N layers ready)
    KV_TRANSFER_COMPLETE    // All layers arrived
};
...
struct RequestInfo
{
    ...
    // NEW: Slot allocation for RDMA memory management
    int slot_id = -1;            // Assigned slot on decode node (0-7)
    size_t slot_base_offset = 0; // Base offset for this slot in bytes
